...
def main():
    ...
    print('useful information:',type(train_loader)) #cjh: for debug
    # #统计验证集的词频
...
def train(train_loader, encoder, decoder, criterion, encoder_optimizer,decoder_optimizer, epoch, p):
    ...
    error_count_temp = 0
    error_img = []
    back_propogation_error=[]
    for i, (imgs, caps, caplens, img_list) in enumerate(train_loader):
        
        # Move to GPU, if available
        # print('information before cuda', np.shape(imgs), np.shape(caps), np.shape(caplens))
        # print(caps)
        imgs = imgs.to(device)
        caps = caps.to(device)
        ...
        except RuntimeError: # maybe some figures are not compatiable to the code, ignore them
            error_count += 1
            error_count_temp += 1
            error_img.extend(img_list)
            print('cuda error, the name is', img_list)
            # print(img_list)
...
#             print(e.message, e.args)
        ...
        try:
            ...
            print('back propagation cuda error')
            back_propogation_error.extend(img_list)
        ...
        if i % print_freq == 0:
            ...
            if error_count_temp:
                error_count_temp =0
                print("Occur {} times cuda erros".format(error_count))
            print('cuda memory source: ',torch.cuda.get_device_capability(device))
        # if i % save_freq == 0:
        #     save_checkpoint(data_name, epoch, epochs_since_improvement, encoder, decoder,encoder_optimizer,
        #                 decoder_optimizer, 0,0)
        del imgs, scores, caps_sorted, decode_lengths, alphas, sort_ind, loss, targets
        torch.cuda.empty_cache()
    with open("epoch_"+str(epoch)+"_error_imgs.json","w") as f:
        json.dump(error_img,f)
    with open("epoch_"+str(epoch)+"_bg_error_imgs.json","w") as f:
        json.dump(back_propogation_error,f)
...
def validate(val_loader, encoder, decoder, criterion):
    ...
    with torch.no_grad():
        ...
        for i, (imgs, caps, caplens, img_list) in enumerate(val_loader):
